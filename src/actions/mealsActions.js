...
import { CATEGORIES_URL, MEAL_CATEGORIES_URL, MEAL_SINGLE_URL, SEARCH_URL, RANDOM_URL} from "../utils/constants";
...
export const startFetchMealsRandom = async(dispatch) => {
    try{
        dispatch({ type: FETCH_RANDOM_BEGIN});
        const response = await axios.get(`${RANDOM_URL}`);
        dispatch({ type: FETCH_RANDOM_SUCCESS, payload: response.data.meals});
    } catch(error){
        dispatch({type: FETCH_RANDOM_ERROR, payload: error.message});
    }
}